...
const translations = {
  en: {
    title: "Workflow",
    merchantsActive: "merchants active",
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[11px] tracking-[-0.32px] ${data.connected ? "text-[#34A853]" : "text-[#999999]"}`}>
              {data.connected ? data.storeName ?? data.t.connected : data.t.disconnected}
            </span>
          </div>
        </div>
      </div>
      <div className="mt-2 text-[11px] tabular-nums tracking-[-0.32px] text-[#666666]">
        {data.orderCount.toLocaleString("en-US")} {data.t.orders}
      </div>
      <Handle
        type="source"
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] tracking-[-0.32px] text-[#918DF6]">{data.t.processingEngine}</p>
        </div>
      </div>
      <div className="mt-3 flex items-center gap-3 text-[11px] tabular-nums tracking-[-0.32px]">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[#666666]">{data.processingCount} {data.t.queued}</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="inline-block size-1.5 rounded-full bg-[#34A853]" />
          <span className="text-[#666666]">{data.deliveredToday.toLocaleString("en-US")} {data.t.today}</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="inline-block size-1.5 rounded-full bg-[#D93025]" />
          <span className="text-[#666666]">{data.failedCount} {data.t.failed}</span>
        </div>
      </div>
      <Handle
        type="source"
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[11px] tracking-[-0.32px] ${data.status === "active" ? "text-[#34A853]" : "text-[#999999]"}`}>
              {data.status === "active" ? data.t.active : data.t.idle}
            </span>
          </div>
        </div>
      </div>
      <div className="mt-2 text-[11px] tabular-nums tracking-[-0.32px] text-[#666666]">
        {data.deliveredCount.toLocaleString("en-US")} {data.t.delivered}
      </div>
    </div>
  )
})
...
const ProviderNode = memo(({ data }: NodeProps<Node<ProviderNodeData>>) => {
  return (
    ...
    >
      ...
      <div className="mt-2 text-[11px] tabular-nums tracking-[-0.32px] text-[#666666]">
        {data.keysAvailable.toLocaleString("en-US")} {data.t.keysAvailable}
      </div>
      <Handle
        type="source"
        position={Position.Right}
...
function getInitialNodes(t: TranslationStrings): Node[] {
  return [
    {
      id: "merchant-naver",
      type: "merchant",
      position: { x: 50, y: 30 },
      data: { label: "네이버 스토어", iconBg: "#03C75A", iconLabel: "N", connected: true, orderCount: 1247, storeName: "Vexora 디지털스토어", t },
    },
    {
      id: "merchant-g2g",
      type: "merchant",
      position: { x: 50, y: 155 },
      data: { label: "롯데몰", iconBg: "#E87A2A", iconLabel: "롯데몰", connected: true, orderCount: 892, storeName: "VexoraDigital", t },
    },
    {
      id: "merchant-g2a",
      type: "merchant",
      position: { x: 50, y: 280 },
      data: { label: "지마켓", iconBg: "#F05A23", iconLabel: "지마켓", connected: false, orderCount: 0, t },
    },
    {
      id: "merchant-direct",
      type: "merchant",
      position: { x: 50, y: 405 },
      data: { label: "Direct / Website", iconBg: "#918DF6", iconLabel: "globe", connected: false, orderCount: 0, t },
    },
    {
      id: "provider-kinguin",
      type: "provider",
      position: { x: 320, y: 40 },
      data: { label: "Kinguin", iconBg: "#0EA5E9", iconLabel: "K", keysAvailable: 4820, status: "active", t },
    },
    {
      id: "provider-keygen",
      type: "provider",
      position: { x: 320, y: 144 },
      data: { label: "Key Generator", iconBg: "#918DF6", iconLabel: "Kg", keysAvailable: 12500, status: "active", t },
    },
    {
      id: "engine",
      type: "engine",
      position: { x: 480, y: 210 },
      data: { label: "Linko", processingCount: 12, deliveredToday: 847, failedCount: 3, t },
    },
    {
      id: "delivery-telegram",
      type: "delivery",
      position: { x: 860, y: 50 },
      data: { label: "Telegram", color: "#2AABEE", deliveredCount: 1423, status: "active", t },
    },
    {
      id: "delivery-email",
      type: "delivery",
      position: { x: 860, y: 165 },
      data: { label: "Email", color: "#2C78FC", deliveredCount: 2891, status: "active", t },
    },
    {
      id: "delivery-sms",
      type: "delivery",
      position: { x: 860, y: 280 },
      data: { label: "SMS", color: "#33C758", deliveredCount: 567, status: "active", t },
    },
    {
      id: "delivery-kakao",
      type: "delivery",
      position: { x: 860, y: 395 },
      data: { label: "KakaoTalk", color: "#FEE500", deliveredCount: 1204, status: "active", t },
    },
  ]
}
...
export default function WorkflowPage({ locale = "en" }: { locale?: Locale }) {
          ...
          <div className="flex items-center gap-6 text-[13px] tracking-[-0.32px]">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[#666666]">
                <span className="tabular-nums font-medium text-[#181925]">{activeMerchants}</span> {t.merchantsActive}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="inline-block size-1.5 rounded-full bg-[#1A73E8]" />
              <span className="text-[#666666]">
                <span className="tabular-nums font-medium text-[#181925]">{totalOrders.toLocaleString("en-US")}</span> {t.totalOrders}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="inline-block size-1.5 rounded-full bg-[#918DF6]" />
              <span className="text-[#666666]">
                <span className="tabular-nums font-medium text-[#181925]">{activeChannels}</span> {t.deliveryChannels}
              </span>
            </div>
          </div>
          <button
