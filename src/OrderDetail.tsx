      ...
      <DashboardLayout title={locale === "kr" ? "주문 상세" : "Order Detail"} locale={locale} currency={currency} onCurrencyToggle={() => setCurrency(currency === "USD" ? "KRW" : "USD")}>
        <div className="flex flex-1 flex-col items-center justify-center gap-4">
          ...
          <button
            ...
            className="inline-flex h-9 items-center gap-2 rounded-full border border-[rgba(0,0,0,0.08)] px-4 text-[13px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.04)]"
          >
  ...
  const WorkflowMessagesSection = () => (
    <div className="mb-6">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-[13px] font-semibold  text-[#666666]">{locale === "kr" ? "발송 메시지" : "Messages"}</p>
        <button
          ...
          className="flex h-7 items-center gap-1.5 rounded-lg border border-[rgba(0,0,0,0.08)] bg-white px-2.5 text-[12px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.03)]"
        >
            ...
            <div key={msg.id} className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[14px] font-semibold  text-[#181925]">{locale === "kr" ? cfg.labelKr : cfg.label}</p>
                  <span className={`shrink-0 rounded-md px-2 py-0.5 text-[11px] font-semibold  ${msg.status === "sent" ? "bg-[#34A853] text-white" : msg.status === "failed" ? "bg-[#D93025] text-white" : "bg-[#E37400] text-white"}`}>
                    {msg.status === "sent" ? (locale === "kr" ? "발송됨" : "Sent") : msg.status === "failed" ? (locale === "kr" ? "실패" : "Failed") : (locale === "kr" ? "대기" : "Pending")}
                  </span>
                </div>
                <p className="mt-1.5 text-[12px]  text-[#999999]">{msg.time}</p>
              </div>
            </div>
          )
        })}
  ...
  const TimelineContent = () => (
    <div className="relative flex flex-col">
      ...
      <div className="relative flex items-start gap-3 pb-5">
        <span className="relative z-10 mt-[4px] size-[14px] shrink-0 rounded-full border-[2.5px] border-[#1A73E8] bg-white" />
        <div>
          <p className="text-[15px] font-semibold  text-[#181925]">{t.orderCreated}</p>
      ...
      <div className="relative flex items-start gap-3 pb-5">
        <span className={`relative z-10 mt-[4px] size-[14px] shrink-0 rounded-full border-[2.5px] bg-white ${order.status === "Failed" && order.errorStep?.includes("Key") ? "border-[#D93025]" : "border-[#34A853]"}`} />
        <div>
          <p className="text-[15px] font-semibold  text-[#181925]">
          ...
          {order.status === "Failed" && order.errorStep?.includes("Key") && order.errorMessage && (
            <p className="mt-1 text-[13px]  text-[#D93025]">{order.errorMessage}</p>
          )}
        </div>
      </div>
      <div className="relative flex items-start gap-3 pb-5">
        <span className={`relative z-10 mt-[4px] size-[14px] shrink-0 rounded-full border-[2.5px] bg-white ${order.status === "Failed" ? "border-[#D93025]" : order.status === "Processing" ? "border-[#E37400]" : "border-[#34A853]"}`} />
        <div>
          <p className="text-[15px] font-semibold  text-[#181925]">
      ...
      {order.status === "Delivered" && order.flowTiming?.deliveryConfirmed && (
        <div className="relative flex items-start gap-3">
          <span className="relative z-10 mt-[4px] size-[14px] shrink-0 rounded-full border-[2.5px] border-[#34A853] bg-white" />
          <div>
            <p className="text-[15px] font-semibold  text-[#181925]">{t.deliveryConfirmed}</p>
            <p className="mt-0.5 text-[13px]  text-[#999999]">
              {formatStepTime(order.time, order.flowTiming.deliveryConfirmed)} · {t.totalTime} {order.flowTiming.deliveryConfirmed}
            </p>
            <p className="mt-0.5 text-[13px] font-medium  text-[#34A853]">{t.normalDelivery}</p>
          </div>
        </div>
      )}
    </div>
  ...
  const RawDataSection = () => (
    <div className="mt-6 border-t border-[rgba(0,0,0,0.08)] pt-4">
      <button
        ...
        className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-[rgba(0,0,0,0.08)] bg-white px-3 text-[12px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.03)]"
      >
  ...
  return (
    <DashboardLayout title={locale === "kr" ? "주문 상세" : "Order Detail"} locale={locale} currency={currency} onCurrencyToggle={() => setCurrency(currency === "USD" ? "KRW" : "USD")}>
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col overflow-hidden">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate(locale === "kr" ? "/kr/dashboard/orders" : "/dashboard/orders")} className="flex items-center gap-1.5 text-[13px] font-medium  text-[#666666] transition-colors hover:text-[#181925]">
              <ChevronLeft className="size-4" strokeWidth={2} />
              {locale === "kr" ? "주문" : "Orders"}
            </button>
            <span className="text-[#E0E0E0]">/</span>
            <h1 className="text-[16px] font-bold  text-[#181925]">Order #{order.id}</h1>
              ...
              {statusEditing && (
                <div className="absolute top-full left-0 z-50 mt-1.5 flex flex-col rounded-lg border border-[rgba(0,0,0,0.08)] bg-white py-1 shadow-lg">
                  {(["Delivered", "Processing", "Failed"] as const).map((s) => (
                    <button key={s} onClick={() => setStatusEditing(false)} className={`flex items-center gap-2 px-3 py-1.5 text-[12px] font-medium  transition-colors hover:bg-[rgba(0,0,0,0.04)] ${s === order.status ? "text-[#181925]" : "text-[#666666]"}`}>
          ...
          <button
            onClick={() => { setSendMsgType(null); setSendMsgChannel(order.delivery); setSendMsgTarget(order.deliveryTarget || order.email || ""); setSendMsgContent(""); setSendMsgRunning(false); setSendMsgDone(null); setSendMsgOpen(true) }}
            className="flex h-8 items-center gap-1.5 rounded-full bg-[#181925] px-4 text-[13px] font-medium  text-white transition-colors hover:bg-[#2a2b3d]"
          >
        ...
        <div className="flex flex-1 gap-4 overflow-hidden lg:gap-5">
            ...
            <div className="flex items-center gap-0 border-b border-[rgba(0,0,0,0.08)]">
              <button onClick={() => setSourcePopupOpen(true)} className="flex flex-1 items-center gap-3 px-5 py-4 text-left transition-colors hover:bg-[rgba(0,0,0,0.02)]">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase  text-[#999999]">{t.source}</p>
                  <p className="mt-0.5 truncate text-[15px] font-semibold  text-[#181925]">{order.storeName || order.platform}</p>
                </div>
              </button>
              ...
              <button onClick={() => setLicensePopupOpen(true)} className="flex flex-1 items-center gap-3 px-5 py-4 text-left transition-colors hover:bg-[rgba(0,0,0,0.02)]">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase  text-[#999999]">{t.license}</p>
                  <p className={`mt-0.5 truncate text-[15px] font-semibold  text-[#181925] ${primaryItemType === "key" || primaryItemType === "giftcard" ? "font-mono" : ""}`}>
              ...
              <button onClick={() => setChannelExpanded(true)} className={`flex flex-1 items-center gap-3 px-5 py-4 text-left transition-colors hover:bg-[rgba(0,0,0,0.02)] ${order.status === "Failed" ? "bg-[#D93025]/[0.02]" : ""}`}>
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase  text-[#999999]">{t.channel}</p>
                  <div className="mt-0.5 flex items-center gap-1.5">
                    <p className="truncate text-[15px] font-semibold  text-[#181925]">{order.deliveryTarget || order.delivery}</p>
            ...
            <div className="flex flex-col gap-0 divide-y divide-[rgba(0,0,0,0.08)]">
              ...
              <div className="p-5">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-[12px] font-semibold uppercase  text-[#999999]">{t.productLabel}</p>
                    <p className="mt-2 text-[16px] font-semibold  text-[#181925]">{order.product}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[12px]  text-[#999999]">{locale === "kr" ? "판매금액" : "Sale"}</p>
                    <p className="text-[17px] font-bold tabular-nums  text-[#181925]">{locale === "kr" ? formatKRW(order.amount) : formatUSD(order.amount)}</p>
                  </div>
                </div>
                <div className="mt-3 flex items-center gap-4 text-[13px]  text-[#666666]">
                  {order.amount > 0 && (
                    <>
              ...
              <div className="p-5">
                <div className="flex items-center justify-between">
                  <p className="text-[12px] font-semibold uppercase  text-[#999999]">{t.buyer}</p>
                  {!buyerEditing && <button onClick={() => setBuyerEditing(true)} className="text-[13px] font-medium  text-[#918DF6] hover:underline">{t.edit}</button>}
                </div>
                {buyerEditing ? (
                  <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3">
                    <div><p className="text-[12px]  text-[#999999]">{t.name}</p><input defaultValue={order.customer} className="mt-1 h-8 w-full rounded-md border border-[rgba(0,0,0,0.12)] bg-white px-2.5 text-[14px]  text-[#181925] outline-none" /></div>
                    <div><p className="text-[12px]  text-[#999999]">{t.phone}</p><input defaultValue={order.phone || ""} className="mt-1 h-8 w-full rounded-md border border-[rgba(0,0,0,0.12)] bg-white px-2.5 text-[14px]  text-[#181925] outline-none" /></div>
                    <div className="col-span-2"><p className="text-[12px]  text-[#999999]">{t.email}</p><input defaultValue={order.email} className="mt-1 h-8 w-full rounded-md border border-[rgba(0,0,0,0.12)] bg-white px-2.5 text-[14px]  text-[#181925] outline-none" /></div>
                    <div className="col-span-2 flex justify-end gap-2"><button onClick={() => setBuyerEditing(false)} className="h-8 rounded-lg px-3 text-[13px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.04)]">{t.cancel}</button><button onClick={() => { console.log("Save buyer", { name: order.customer, phone: order.phone, email: order.email }); setBuyerEditing(false) }} className="h-8 rounded-lg bg-[#181925] px-3 text-[13px] font-medium  text-white transition-colors hover:bg-[#2a2b3a]">{t.save}</button></div>
                  </div>
                ) : (
                  <div className="mt-3 grid grid-cols-3 gap-4">
                    <div><p className="text-[12px]  text-[#999999]">{t.name}</p><p className="mt-1 text-[14px] font-medium  text-[#181925]">{order.customer}</p></div>
                    <div><p className="text-[12px]  text-[#999999]">{t.phone}</p><p className="mt-1 text-[14px] font-medium  text-[#181925]">{order.phone || "—"}</p></div>
                    <div><p className="text-[12px]  text-[#999999]">{t.email}</p><p className="mt-1 truncate text-[14px] font-medium  text-[#181925]">{order.email}</p></div>
                  </div>
                )}
              </div>
              ...
              {/* Delivery Details */}
              <div className="p-5">
                <p className="text-[12px] font-semibold uppercase  text-[#999999]">{t.deliveryDetails}</p>
                <div className="mt-3 rounded-xl border border-[rgba(0,0,0,0.08)] bg-[rgba(0,0,0,0.015)]">
                  <div className="flex items-center gap-3 border-b border-[rgba(0,0,0,0.06)] px-4 py-3.5">
                    ...
                    <div className="flex-1">
                      <p className="text-[16px] font-semibold  text-[#181925]">{order.delivery}</p>
                      <p className="text-[14px]  text-[#666666]">{order.deliveryTarget || "—"}</p>
                    </div>
                    <div className="flex items-center gap-1.5 rounded-full border border-[rgba(0,0,0,0.06)] bg-white px-3 py-1">
                      ...
                      <span className="text-[13px] font-medium  text-[#181925]">{order.status === "Delivered" ? t.delivered : order.status === "Processing" ? t.pending : t.failed}</span>
                    </div>
                  </div>
                  <div className="px-4 py-3">
                    <p className="text-[14px]  text-[#666666]"><span className="font-medium text-[#181925]">{order.recipientName || order.customer}</span>{" · "}{order.time}{" · "}{order.storeName || order.platform}</p>
                  </div>
                  {order.status === "Failed" && order.errorMessage && (
                    <div className="border-t border-[rgba(0,0,0,0.06)] px-4 py-3">
                      <div className="rounded-md border border-[#D93025]/15 bg-[#D93025]/[0.04] px-3 py-2.5">
                        <p className="text-[13px] font-semibold  text-[#D93025]">{order.errorStep}</p>
                        <p className="mt-1 text-[13px] leading-relaxed  text-[#666666]">{order.errorMessage}</p>
                      </div>
                    </div>
                  )}
                  <div className="border-t border-[rgba(0,0,0,0.06)] px-4 py-3">
              ...
              <div className="p-5">
                <div className="flex items-center gap-2">
                  <p className="text-[12px] font-semibold uppercase  text-[#999999]">{t.licenseLabel}</p>
                  {qty > 1 && <span className="rounded-full bg-[rgba(0,0,0,0.06)] px-2 py-0.5 text-[12px] font-semibold tabular-nums  text-[#666666]">{qty}</span>}
                </div>
                    ...
                    if (itemType === "file") {
                      return (
                        <div key={idx} className="flex items-center gap-2.5 rounded-lg border border-[rgba(0,0,0,0.08)] bg-[rgba(0,0,0,0.02)] px-3.5 py-2.5">
                          ...
                          <span className="flex-1 truncate text-[13px] font-medium  text-[#181925]">{item.label || item.keyCode}</span>
                          <button onClick={() => console.log("Download file", item.keyCode)} className="flex h-7 shrink-0 items-center gap-1 rounded-md border border-[rgba(0,0,0,0.08)] bg-white px-2.5 text-[12px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.03)]"><Download className="size-3" strokeWidth={2} />{locale === "kr" ? "다운로드" : "Download"}</button>
                        </div>
                      )
                    }
                    if (itemType === "link") {
                      return (
                        <div key={idx} className="flex items-center gap-2.5 rounded-lg border border-[rgba(0,0,0,0.08)] bg-[rgba(0,0,0,0.02)] px-3.5 py-2.5">
                          ...
                          <span className="flex-1 truncate text-[13px] font-medium  text-[#181925]">{item.label || item.keyCode}</span>
                          <a href={item.keyCode} target="_blank" rel="noreferrer" className="flex h-7 shrink-0 items-center gap-1 rounded-md border border-[rgba(0,0,0,0.08)] bg-white px-2.5 text-[12px] font-medium  text-[#1A73E8] transition-colors hover:bg-[rgba(26,115,232,0.04)]"><ExternalLink className="size-3" strokeWidth={2} />{locale === "kr" ? "열기" : "Open"}</a>
                        </div>
                      )
                    }
                    if (itemType === "giftcard") {
                      return (
                        <div key={idx} className="flex items-center gap-2.5 rounded-lg border border-[rgba(255,180,0,0.25)] bg-[rgba(255,224,0,0.06)] px-3.5 py-2.5">
                          ...
                          <span className="flex-1 truncate font-mono text-[13px]  text-[#181925]">{item.keyCode}</span>
                ...
                {order.keyCode.endsWith("0000") && <p className="mt-2 text-[12px]  text-[#E37400]">{t.keyPlaceholderWarning}</p>}
              ...
              {/* Admin Memo */}
              <div className="p-5">
                <p className="text-[12px] font-semibold uppercase  text-[#999999]">{t.adminMemo}</p>
                <textarea defaultValue={order.adminMemo || ""} placeholder={t.addNote} rows={memoFocused || (order.adminMemo && order.adminMemo.length > 0) ? 3 : 1} onFocus={() => setMemoFocused(true)} onBlur={() => setMemoFocused(false)} onChange={handleMemoChange} className="mt-2 w-full resize-none rounded-lg border border-[rgba(0,0,0,0.12)] bg-white px-3 py-2.5 text-[13px]  text-[#181925] placeholder:text-[#999999] outline-none transition-all" />
                <div className="mt-1.5 flex items-center gap-1.5">
                  {memoSaveStatus === "saving" && <div className="flex items-center gap-1"><Loader2 className="size-3 animate-spin text-[#999999]" strokeWidth={2} /><p className="text-[12px]  text-[#999999]">{t.saving}</p></div>}
                  {memoSaveStatus === "saved" && <div className="flex items-center gap-1"><Check className="size-3 text-[#34A853]" strokeWidth={2.5} /><p className="text-[12px]  text-[#34A853]">{t.autoSaved}</p></div>}
                  {memoSaveStatus === "idle" && <p className="text-[12px]  text-[#999999]">{t.autoSave}</p>}
                </div>
              </div>

              {/* Danger Zone */}
              <div className="p-5">
                <p className="text-[12px] font-semibold uppercase  text-[#999999]">{t.dangerZone}</p>
                <div className="mt-3 flex gap-2">
                  <button onClick={() => console.log("Cancel order", order.id)} className="inline-flex h-9 items-center rounded-lg border border-[rgba(0,0,0,0.08)] px-4 text-[13px] font-medium  text-[#D93025] transition-colors hover:bg-[#D93025]/[0.04]">{t.cancelOrder}</button>
                  <button onClick={() => console.log("Delete order", order.id)} className="inline-flex h-9 items-center rounded-lg bg-[#D93025] px-4 text-[13px] font-medium  text-white transition-colors hover:bg-[#c12b20]">{t.deleteOrder}</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="hidden lg:flex w-[340px] shrink-0 flex-col overflow-y-auto rounded-xl border border-[rgba(0,0,0,0.08)] bg-white" style={{ boxShadow: "0 1px 1px rgba(0,0,0,0.08), 0 0 0 0.5px rgba(0,0,0,0.06)" }}>
            <div className="border-b border-[rgba(0,0,0,0.08)] px-5 py-4">
              <p className="text-[15px] font-semibold  text-[#181925]">{locale === "kr" ? "활동" : "Activity"}</p>
            </div>
            <div className="flex-1 overflow-y-auto p-5">
              <WorkflowMessagesSection />
              <div className="mb-5 border-t border-[rgba(0,0,0,0.06)]" />
              <p className="mb-3 text-[13px] font-semibold  text-[#666666]">{locale === "kr" ? "타임라인" : "Timeline"}</p>
              <TimelineContent />
              <RawDataSection />
            </div>
          </div>
        ...
        <div className="mt-4 lg:hidden rounded-xl border border-[rgba(0,0,0,0.08)] bg-white p-5" style={{ boxShadow: "0 1px 1px rgba(0,0,0,0.08), 0 0 0 0.5px rgba(0,0,0,0.06)" }}>
          <p className="mb-4 text-[13px] font-semibold  text-[#181925]">{locale === "kr" ? "활동" : "Activity"}</p>
          <WorkflowMessagesSection />
          <div className="mb-5 border-t border-[rgba(0,0,0,0.06)]" />
          <p className="mb-3 text-[12px] font-semibold  text-[#999999]">{locale === "kr" ? "타임라인" : "Timeline"}</p>
          <TimelineContent />
          <RawDataSection />
        </div>
      </div>
        ...
        <DialogContent className="sm:max-w-md" showCloseButton>
          <DialogHeader>
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <DialogTitle className="text-[18px] font-semibold  text-[#181925]">{order.platform}</DialogTitle>
                <DialogDescription className="text-[14px]  text-[#999999]">{order.storeName || order.platform}</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-2.5 rounded-lg bg-[rgba(0,0,0,0.02)] px-4 py-3">
              <div className="flex items-center justify-between"><span className="text-[13px]  text-[#666666]">{t.orderIdLabel}</span><span className="font-mono text-[13px] font-medium tabular-nums  text-[#181925]">#{order.id}</span></div>
              <div className="flex items-center justify-between gap-3"><span className="text-[13px]  text-[#666666]">{t.customerLabel}</span><span className="text-[13px] font-medium  text-[#181925]">{order.customer}</span></div>
              <div className="flex items-center justify-between gap-3"><span className="text-[13px]  text-[#666666]">{t.amountLabel}</span><span className="text-[13px] font-medium tabular-nums  text-[#181925]">{locale === "kr" ? formatKRW(order.amount) : formatUSD(order.amount)}</span></div>
              {order.flowTiming && <div className="flex items-center justify-between gap-3"><span className="text-[13px]  text-[#666666]">{t.orderReceived}</span><span className="font-mono text-[13px] tabular-nums  text-[#181925]">{order.flowTiming.orderCreated}</span></div>}
            </div>
            <div className="flex flex-col gap-2">
              <button onClick={() => { setSourcePopupOpen(false); navigate("/dashboard/merchants") }} className="flex h-10 w-full items-center justify-center gap-1.5 rounded-full bg-[#918DF6] text-[14px] font-medium  text-white transition-colors hover:bg-[#7B77E0]">{t.goToMerchants}</button>
              <button onClick={() => { setSourcePopupOpen(false); window.open(`https://${order.platform}`, '_blank') }} className="flex h-10 w-full items-center justify-center gap-1.5 rounded-full border border-[rgba(0,0,0,0.08)] text-[14px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.03)]">{t.viewOn(order.platform)}</button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={licensePopupOpen} onOpenChange={(open) => { if (!open) { setLicensePopupOpen(false); setCopiedLicenseKey(false) } }}>
        <DialogContent className="sm:max-w-md" showCloseButton>
          <DialogHeader>
            <DialogTitle className="text-[18px] font-semibold  text-[#181925]">{t.licenseDetails}</DialogTitle>
            <DialogDescription className="text-[14px]  text-[#999999]">{order.product}</DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-3">
            {items.map((item, idx) => (
              <div key={item.keyCode} className="flex items-center justify-between rounded-lg bg-[rgba(0,0,0,0.02)] px-4 py-3">
                <div className="min-w-0 flex-1">
                  {qty > 1 && <p className="text-[11px] font-medium  text-[#999999]">Key {idx + 1}</p>}
            ...
            {order.flowTiming && <div className="flex flex-col gap-2.5 rounded-lg bg-[rgba(0,0,0,0.02)] px-4 py-3"><div className="flex items-center justify-between"><span className="text-[13px]  text-[#666666]">{t.assignedAt}</span><span className="font-mono text-[13px] tabular-nums  text-[#181925]">{order.flowTiming.licenseAssigned}</span></div></div>}
            <div className="flex flex-col gap-2">
              <button onClick={() => { setLicensePopupOpen(false); setRetryDialogOpen(true) }} className="flex h-10 w-full items-center justify-center gap-1.5 rounded-full bg-[#918DF6] text-[14px] font-medium  text-white transition-colors hover:bg-[#7B77E0]"><RotateCcw className="size-4" strokeWidth={2} />{t.retryAssignment}</button>
              <button onClick={() => { setLicensePopupOpen(false); setReassignDialogOpen(true) }} className="flex h-10 w-full items-center justify-center gap-1.5 rounded-full border border-[rgba(0,0,0,0.08)] text-[14px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.03)]">{t.reassignLicense}</button>
              <button onClick={() => { setLicensePopupOpen(false); navigate("/dashboard/inventory") }} className="flex h-10 w-full items-center justify-center gap-1.5 rounded-full border border-[rgba(0,0,0,0.08)] text-[14px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.03)]">{t.viewInInventory}</button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-sm" showCloseButton>
          <DialogHeader>
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <DialogTitle className="text-[16px] font-semibold  text-[#181925]">{order.delivery} Delivery</DialogTitle>
                <DialogDescription className="text-[13px]  text-[#999999]">{order.deliveryTarget || "—"}</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          <div className="flex flex-col gap-2.5">
            <div className="flex items-center justify-between rounded-lg bg-[rgba(0,0,0,0.02)] px-3.5 py-2.5"><span className="text-[13px]  text-[#666666]">{t.statusLabel}</span><StatusBadge status={order.status} locale={locale} /></div>
            {order.flowTiming && <div className="flex flex-col gap-2 rounded-lg bg-[rgba(0,0,0,0.02)] px-3.5 py-2.5"><div className="flex items-center justify-between"><span className="text-[13px]  text-[#666666]">{t.sentAt}</span><span className="font-mono text-[13px] tabular-nums  text-[#181925]">{order.flowTiming.deliverySent}</span></div>{order.flowTiming.deliveryConfirmed && <div className="flex items-center justify-between"><span className="text-[13px]  text-[#666666]">{t.confirmedAt}</span><span className="font-mono text-[13px] tabular-nums  text-[#34A853]">{order.flowTiming.deliveryConfirmed}</span></div>}</div>}
            <div className="flex flex-col gap-2">
              <button onClick={() => { setChannelExpanded(false); setChannelResendConfirm(true) }} className="flex h-9 w-full items-center justify-center gap-1.5 rounded-full text-[13px] font-medium  text-white transition-colors" style={{ backgroundColor: ch?.color || "#181925" }}><Send className="size-3.5" strokeWidth={2} />{t.resend}</button>
              <button onClick={() => { setChannelExpanded(false); setMessagePopupOpen(true) }} className="flex h-9 w-full items-center justify-center gap-1.5 rounded-full border border-[rgba(0,0,0,0.08)] text-[13px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.03)]">{t.viewMessage}</button>
              <button onClick={() => { setChannelExpanded(false); setRawDataPopupOpen(true) }} className="flex h-9 w-full items-center justify-center gap-1.5 rounded-full border border-[rgba(0,0,0,0.08)] text-[13px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.03)]">{t.viewRawData}</button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={messagePopupOpen} onOpenChange={setMessagePopupOpen}>
        <DialogContent className="sm:max-w-sm" showCloseButton>
          <DialogHeader>
            <DialogTitle className="text-[16px] font-semibold  text-[#181925]">{t.messageTitle}</DialogTitle>
            <DialogDescription className="text-[13px]  text-[#999999]">#{order.id} · {order.delivery}</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={rawDataPopupOpen} onOpenChange={setRawDataPopupOpen}>
        <DialogContent className="sm:max-w-sm" showCloseButton>
          <DialogHeader>
            <DialogTitle className="text-[16px] font-semibold  text-[#181925]">Raw Data</DialogTitle>
            <DialogDescription className="text-[13px]  text-[#999999]">Order #{order.id}</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={channelResendConfirm} onOpenChange={(open) => { if (!open) { setChannelResendConfirm(false); setResendRunning(false); setResendDone(null) } }}>
        <DialogContent className="sm:max-w-sm" showCloseButton>
          <DialogHeader>
            <DialogTitle className="text-[16px] font-semibold  text-[#181925]">{resendDone === "success" ? t.resendComplete : resendDone === "failed" ? t.resendFailedTitle : resendRunning ? t.resending : t.resendConfirmTitle}</DialogTitle>
            <DialogDescription className="text-[13px]  text-[#999999]">{resendDone === "success" ? t.resendCompleteDesc : resendDone === "failed" ? t.resendFailedDesc : resendRunning ? t.resendingDesc : t.resendConfirmDesc}</DialogDescription>
          </DialogHeader>
          {resendRunning && !resendDone && <div className="flex flex-col items-center gap-3 py-6"><Loader2 className="size-8 animate-spin" style={{ color: ch?.color || "#918DF6" }} strokeWidth={2} /><p className="text-[13px]  text-[#666666]">{t.resendingChannel(order.delivery)}</p></div>}
          {resendDone === "success" && <div className="flex flex-col items-center gap-3 py-6"><span className="flex size-12 items-center justify-center rounded-full bg-[rgba(52,168,83,0.1)]"><Check className="size-6 text-[#34A853]" strokeWidth={2} /></span><div className="text-center"><p className="text-[14px] font-medium  text-[#181925]">{t.sendComplete}</p><p className="mt-1 text-[12px]  text-[#666666]">{t.sendCompleteDesc(order.deliveryTarget || "")}</p></div></div>}
          {resendDone === "failed" && <div className="flex flex-col items-center gap-3 py-6"><span className="flex size-12 items-center justify-center rounded-full bg-[rgba(217,48,37,0.1)]"><X className="size-6 text-[#D93025]" strokeWidth={2} /></span><div className="text-center"><p className="text-[14px] font-medium  text-[#181925]">{t.sendFailed}</p><p className="mt-1 text-[12px]  text-[#666666]">{t.channelError}</p></div></div>}
          {!resendRunning && !resendDone && (
            <>
              <div className="flex flex-col gap-3 rounded-lg border border-[rgba(0,0,0,0.08)] bg-[rgba(0,0,0,0.02)] p-4">
                <div className="flex items-center gap-2">{ch && <span style={{ color: ch.color }}>{ch.icon}</span>}<span className="text-[15px] font-medium  text-[#181925]">{order.delivery}</span><span className={`size-2 rounded-full ${statusDot}`} /></div>
                <div className="flex flex-col gap-2 text-[14px]  text-[#666666]">
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button onClick={() => setChannelResendConfirm(false)} className="h-8 rounded-lg px-3 text-[12px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.04)]">{t.cancelBtn}</button>
                <button onClick={() => { setResendRunning(true); setTimeout(() => { setResendRunning(false); setResendDone(Math.random() > 0.2 ? "success" : "failed") }, 2000) }} className="h-8 rounded-lg px-3 text-[12px] font-medium  text-white transition-colors" style={{ backgroundColor: ch?.color || "#181925" }}>{t.resendBtn}</button>
              </div>
            </>
          )}
          {resendDone && <div className="flex justify-end pt-1"><button onClick={() => { setChannelResendConfirm(false); setResendRunning(false); setResendDone(null) }} className="h-8 rounded-lg px-3 text-[12px] font-medium  text-white transition-colors bg-[#918DF6] hover:bg-[#7B77E0]">{t.confirm}</button></div>}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={retryDialogOpen} onOpenChange={(open) => { if (!open) { setRetryDialogOpen(false); setRetryRunning(false); setRetryDone(false) } }}>
        <DialogContent className="sm:max-w-sm" showCloseButton>
          <DialogHeader>
            <DialogTitle className="text-[16px] font-semibold  text-[#181925]">{retryDone ? t.retryComplete : retryRunning ? t.retrying : t.retryLicenseAssignment}</DialogTitle>
            <DialogDescription className="text-[13px]  text-[#666666]">{retryDone ? t.retryCompleteDesc : retryRunning ? t.retryingDesc : t.retryDesc}</DialogDescription>
          </DialogHeader>
          {retryRunning && !retryDone && <div className="flex flex-col items-center gap-3 py-4"><Loader2 className="size-8 animate-spin text-[#918DF6]" strokeWidth={2} /><p className="text-[13px]  text-[#666666]">{t.assigningLicense}</p></div>}
          {retryDone && <div className="flex flex-col gap-3"><div className="rounded-lg border border-[#34A853]/15 bg-[#34A853]/[0.04] px-4 py-3"><div className="flex items-center gap-2"><Check className="size-4 text-[#34A853]" strokeWidth={2.5} /><p className="text-[13px] font-medium  text-[#34A853]">{t.licenseAssignedSuccess}</p></div><div className="mt-2 flex flex-col gap-1 text-[12px]  text-[#666666]"><div className="flex justify-between gap-3"><span>{t.product}</span><span className="font-medium text-[#181925]">{order.product}</span></div><div className="flex justify-between gap-3"><span>{t.keyLabel}</span><span className="font-mono font-medium text-[#181925]">{order.keyCode}</span></div><div className="flex justify-between gap-3"><span>{t.duration}</span><span className="font-medium text-[#181925]">0.4s</span></div></div></div></div>}
          <div className="flex justify-end gap-2 pt-1">
            {retryDone ? <button onClick={() => { setRetryDialogOpen(false); setRetryRunning(false); setRetryDone(false) }} className="h-8 rounded-lg bg-[#918DF6] px-4 text-[12px] font-medium  text-white transition-colors hover:bg-[#7B77E0]">{t.done}</button> : retryRunning ? null : (
              <>
                <button onClick={() => setRetryDialogOpen(false)} className="h-8 rounded-lg px-3 text-[12px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.04)]">{t.cancel}</button>
                <button onClick={() => { setRetryRunning(true); setTimeout(() => { setRetryRunning(false); setRetryDone(true) }, 2000) }} className="h-8 rounded-lg bg-[#918DF6] px-4 text-[12px] font-medium  text-white transition-colors hover:bg-[#7B77E0]"><span className="flex items-center gap-1.5"><RotateCcw className="size-3" strokeWidth={2} />{t.retryNow}</span></button>
              </>
            )}
          </div>
        </DialogContent>
      ...
      <Dialog open={reassignDialogOpen} onOpenChange={(open) => { if (!open) { setReassignDialogOpen(false); setReassignRunning(false); setReassignDone(false); setReassignSelectedKey([]) } }}>
        <DialogContent className="sm:max-w-sm" showCloseButton>
          <DialogHeader>
            <DialogTitle className="text-[16px] font-semibold  text-[#181925]">{reassignDone ? t.licenseReassigned : t.reassignLicenseTitle}</DialogTitle>
            <DialogDescription className="text-[13px]  text-[#666666]">{reassignDone ? t.reassignedDesc(reassignSelectedKey.length) : t.selectLicenses(order.id)}</DialogDescription>
          </DialogHeader>
          {reassignRunning && !reassignDone && <div className="flex flex-col items-center gap-3 py-4"><Loader2 className="size-8 animate-spin text-[#918DF6]" strokeWidth={2} /><p className="text-[13px]  text-[#666666]">{t.assigningLicense}</p></div>}
          {reassignDone && <div className="rounded-lg border border-[#34A853]/15 bg-[#34A853]/[0.04] px-4 py-3"><div className="flex items-center gap-2"><Check className="size-4 text-[#34A853]" strokeWidth={2.5} /><p className="text-[13px] font-medium  text-[#34A853]">{t.reassigned(reassignSelectedKey.length)}</p></div><div className="mt-2 flex flex-col gap-1 text-[12px]  text-[#666666]">{reassignSelectedKey.map((key) => <div key={key} className="flex justify-between gap-3"><span>{t.keyLabel}</span><span className="font-mono font-medium text-[#181925]">{key}</span></div>)}<div className="flex justify-between gap-3"><span>{t.product}</span><span className="font-medium text-[#181925]">{order.product}</span></div></div></div>}
          {!reassignRunning && !reassignDone && (() => {
            ...
            return (
              <div className="flex flex-col gap-3">
                <p className="text-[11px] font-semibold  text-[#999999]">{t.availableKeys(mockAvailableKeys.length)}</p>
                <div className="flex flex-col gap-1.5">
                  {mockAvailableKeys.map((key) => (
                    <button key={key.code} type="button" onClick={() => setReassignSelectedKey((prev) => prev.includes(key.code) ? prev.filter((k) => k !== key.code) : [...prev, key.code])} className={`flex items-center gap-3 rounded-lg border px-3 py-2.5 text-left transition-colors ${reassignSelectedKey.includes(key.code) ? "border-[#918DF6] bg-[#918DF6]/[0.04]" : "border-[rgba(0,0,0,0.08)] hover:bg-[rgba(0,0,0,0.02)]"}`}>
                      ...
                      <div className="min-w-0 flex-1"><p className="font-mono text-[12px]  text-[#181925]">{key.code}</p><p className="text-[11px]  text-[#999999]">{t.added} {key.addedDate}</p></div>
                      {reassignSelectedKey.includes(key.code) && <Check className="size-4 shrink-0 text-[#918DF6]" strokeWidth={2.5} />}
                    </button>
                  ))}
                </div>
                <div className="rounded-lg border border-[rgba(227,116,0,0.15)] bg-[rgba(227,116,0,0.04)] px-3 py-2"><p className="text-[11px]  text-[#E37400]">{t.currentKeyWarning(order.keyCode)}</p></div>
              </div>
            )
          })()}
          <div className="flex justify-end gap-2 pt-1">
            {reassignDone ? <button onClick={() => { setReassignDialogOpen(false); setReassignRunning(false); setReassignDone(false); setReassignSelectedKey([]) }} className="h-8 rounded-lg bg-[#918DF6] px-4 text-[12px] font-medium  text-white transition-colors hover:bg-[#7B77E0]">{t.done}</button> : reassignRunning ? null : (
              <>
                <button onClick={() => setReassignDialogOpen(false)} className="h-8 rounded-lg px-3 text-[12px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.04)]">{t.cancel}</button>
                <button onClick={() => { setReassignRunning(true); setTimeout(() => { setReassignRunning(false); setReassignDone(true) }, 2000) }} className="h-8 rounded-lg bg-[#918DF6] px-4 text-[12px] font-medium  text-white transition-colors hover:bg-[#7B77E0]" disabled={reassignSelectedKey.length === 0}>{t.assign(reassignSelectedKey.length)}</button>
              </>
            )}
          </div>
        </DialogContent>
      ...
      <Dialog open={sendMsgOpen} onOpenChange={(open) => { if (!open) { setSendMsgOpen(false); setSendMsgType(null); setSendMsgRunning(false); setSendMsgDone(null) } }}>
        <DialogContent className="sm:max-w-md" showCloseButton>
          <DialogHeader>
            <DialogTitle className="text-[18px] font-semibold  text-[#181925]">
              {sendMsgDone === "success"
            ...
            <DialogDescription className="text-[13px]  text-[#999999]">
              {sendMsgDone === "success"
          ...
          {!sendMsgRunning && !sendMsgDone && (
            <div className="flex flex-col gap-4">
              {/* Step 1: Message type */}
              <div>
                <p className="mb-2 text-[12px] font-semibold  text-[#999999]">{locale === "kr" ? "메시지 유형" : "Type"}</p>
                <div className="flex flex-col gap-1.5">
                  {(["product", "confirm", "announcement"] as const).map((type) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <p className="text-[13px] font-semibold  text-[#181925]">{cfg.label}</p>
                          <p className="mt-0.5 text-[12px]  text-[#999999]">{cfg.desc}</p>
                        </div>
              ...
              {sendMsgType && (
                <>
                  {sendMsgType === "announcement" && (
                    <div>
                      <p className="mb-1.5 text-[12px] font-semibold  text-[#999999]">{locale === "kr" ? "공지 내용" : "Message"}</p>
                      <textarea
                        ...
                        rows={3}
                        className="w-full resize-none rounded-lg border border-[rgba(0,0,0,0.12)] bg-white px-3 py-2.5 text-[13px]  text-[#181925] placeholder:text-[#999999] outline-none transition-colors focus:border-[#918DF6]"
                      />
                    </div>
                  )}

                  <div>
                    <p className="mb-1.5 text-[12px] font-semibold  text-[#999999]">{locale === "kr" ? "받는 사람" : "Recipient"}</p>
                    <input
                      type="text"
                      value={sendMsgTarget}
                      onChange={(e) => setSendMsgTarget(e.target.value)}
                      className="h-10 w-full rounded-lg border border-[rgba(0,0,0,0.12)] bg-white px-3 text-[13px]  text-[#181925] outline-none transition-colors focus:border-[#918DF6]"
                    />
                  </div>

                  <div>
                    <p className="mb-1.5 text-[12px] font-semibold  text-[#999999]">{locale === "kr" ? "채널" : "Channel"}</p>
                      ...
                      {channels.map((chItem) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex flex-col items-center gap-1 rounded-lg border py-2.5 text-[11px] font-medium  transition-all ${selected ? "border-[#181925] bg-[#181925] text-white" : "border-[rgba(0,0,0,0.08)] bg-white text-[#666666] hover:bg-[rgba(0,0,0,0.02)]"}`}
                          >
                  ...
                  <div className="flex justify-end gap-2 pt-1">
                    <button onClick={() => setSendMsgOpen(false)} className="h-8 rounded-lg px-3 text-[12px] font-medium  text-[#666666] transition-colors hover:bg-[rgba(0,0,0,0.04)]">
                      {locale === "kr" ? "취소" : "Cancel"}
                    </button>
                    <button
                      ...
                      className="flex h-8 items-center gap-1.5 rounded-lg bg-[#181925] px-4 text-[12px] font-medium  text-white transition-colors hover:bg-[#2a2b3d] disabled:opacity-40"
                    >
                      <Send className="size-3" strokeWidth={2} />
                      {locale === "kr" ? "발송" : "Send"}
                    </button>
          ...
          {sendMsgDone && (
            <div className="flex justify-end pt-1">
              <button
                ...
                className="h-8 rounded-lg bg-[#918DF6] px-4 text-[12px] font-medium  text-white transition-colors hover:bg-[#7B77E0]"
              >
                {locale === "kr" ? "확인" : "Done"}
              </button>
            </div>
      ...
      <Dialog open={advancedResendOpen} onOpenChange={(open) => { if (!open) setAdvancedResendOpen(false) }}>
        <DialogContent className="sm:max-w-md" showCloseButton>
          <DialogHeader>
            <DialogTitle className="text-[18px] font-semibold  text-[#181925]">{locale === "kr" ? "다시 보내기" : "Resend"}</DialogTitle>
            <DialogDescription className="text-[14px]  text-[#999999]">{locale === "kr" ? "채널과 받는 사람을 선택하세요" : "Choose channel and recipient"}</DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-4">
            <div>
              <p className="text-[13px] font-medium  text-[#666666]">{locale === "kr" ? "보낼 메시지" : "Message"}</p>
              <div className="mt-2 rounded-lg border border-[#E37400]/30 bg-[#E37400]/[0.04] px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-[14px] font-medium  text-[#181925]">{order.product}</p><p className="text-[12px]  text-[#999999]">{order.delivery}</p></div>
                </div>
              </div>
            </div>
            <div>
              <p className="text-[13px] font-medium  text-[#666666]">{locale === "kr" ? "받는 사람" : "Recipient"}</p>
              <input type="text" value={resendTarget} onChange={(e) => setResendTarget(e.target.value)} className="mt-2 h-11 w-full rounded-lg border border-[rgba(0,0,0,0.12)] bg-white px-4 text-[14px]  text-[#181925] outline-none transition-colors focus:border-[#918DF6]" />
              <p className="mt-1.5 text-[12px]  text-[#999999]">{locale === "kr" ? "받는 번호를 바꿔서 보낼 수 있어요." : "You can change the recipient."}</p>
            </div>
            <div>
              <p className="text-[13px] font-medium  text-[#666666]">{locale === "kr" ? "채널 선택" : "Channel"}</p>
              <div className="mt-2 grid grid-cols-2 gap-2">
                {channels.map((chItem) => (
                  <button key={chItem.id} onClick={() => setResendChannel(chItem.id)} className={`flex h-11 items-center justify-center gap-2 rounded-lg border text-[14px] font-medium  transition-all ${resendChannel === chItem.id ? "border-[#181925] bg-[#181925] text-white" : "border-[rgba(0,0,0,0.08)] bg-white text-[#181925] hover:bg-[rgba(0,0,0,0.02)]"}`}>
            ...
            <button onClick={() => { setAdvancedResendOpen(false); setChannelResendConfirm(true) }} className="flex h-11 w-full items-center justify-center gap-2 rounded-full bg-[#181925] text-[14px] font-medium  text-white transition-colors hover:bg-[#2a2b3d]">
